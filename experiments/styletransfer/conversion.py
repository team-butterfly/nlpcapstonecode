...
#         print(repr(b[i]).rjust(7)[:int(log10(np.amax(b)))+5], '| ', '#'*int(70*h[i-1]/np.amax(h)))
#
#     print(repr(b[bins]).rjust(7)[:int(log10(np.amax(b)))+5])
...
def audioFileToSpectrogram(audioFile, fftWindowSize):
    ...
    phase = np.tanh(np.imag(spectrogram)) / 2
    amplitude = np.log1p(np.abs(spectrogram))
    # console.bounds(amplitude, "amplitude")
    # console.bounds(phase, "phase")
    # console.log("starting histogram generation")
